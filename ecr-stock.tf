resource "aws_ecr_repository" "repository_stock" {
  name                 = "ordering-system-stock"
...
resource "aws_ecr_repository_policy" "repository-stock-policy" {
  repository = aws_ecr_repository.repository_stock.name
...
resource "aws_ecr_lifecycle_policy" "repository-stock-lifecycle" {
  repository = aws_ecr_repository.repository_stock.name
...
resource "null_resource" "push_image_stock_to_ecr" {
  provisioner "local-exec" {
    command     = <<-EOT
      ...
      docker build -t ${aws_ecr_repository.repository_stock.repository_url}:latest .
      docker push ${aws_ecr_repository.repository_stock.repository_url}:latest
      rm -rf ./temp_repo_stock
    EOT
    working_dir = path.module
  }
  depends_on = [aws_ecr_repository.repository_stock]
}
