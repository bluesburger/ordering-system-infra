...
resource "aws_ecs_service" "service_invoice" {
  name            = "SVC-${var.project_name_invoice}"
  cluster         = aws_ecs_cluster.cluster.arn
  task_definition = aws_ecs_task_definition.task_invoice.arn
  ...
  load_balancer {
    ...
    container_name   = var.project_name_invoice
    container_port   = 8080
  }
